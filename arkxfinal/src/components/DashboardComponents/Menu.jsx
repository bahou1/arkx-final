...
import { AiOutlineHeart, AiOutlineEdit, AiOutlineEye, AiOutlineDelete, AiOutlineClose, AiOutlineDollar, AiOutlineStar } from 'react-icons/ai';
...
const Menu = () => {
    ...
    const handleSaveEdit = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:3000/api/menu/${editItem.identifier}`, {
                method: 'PUT',
                headers: {
                ...
                <div key={item._id} className="flex flex-col items-center gap-3 relative bg-white rounded-3xl pb-4">
                    ...
                    <div className="flex gap-4 mt-8">
                        <div className="flex flex-col items-center">
                            <button onClick={() => handleEdit(item)} className="rounded-2xl w-12 h-12 font-lato font-3xl flex items-center justify-center transition-colors duration-200 ease-in-out" style={{ backgroundColor: '#FFF0ED', color: '#FF6D4D' }}>
                ...
                <div className="fixed top-0 left-0 w-full h-full flex items-center justify-center bg-gray-900 bg-opacity-50 transition-opacity duration-300">
                    <div className="bg-white p-6 rounded-lg max-w-md transform transition-all duration-300 opacity-100 shadow-lg border-4 border-gray-300 animate-popup">
                        ...
                        <div className="mb-4">
                            ...
                            <input type="text" name="title" value={editItem.title} onChange={handleEditChange} className="w-full p-2 border rounded-lg" />
                        </div>
                        <div className="mb-4">
                            <label className="block text-lg font-medium">Price</label>
                            <input type="text" name="price" value={editItem.price} onChange={handleEditChange} className="w-full p-2 border rounded-lg" />
                        </div>
                        <div className="mb-4">
                            <label className="block text-lg font-medium">Image URL</label>
                            <input type="text" name="imgUrl" value={editItem.imgUrl} onChange={handleEditChange} className="w-full p-2 border rounded-lg" />
                        </div>
                        <div className="flex justify-end">
                            <button onClick={handleCloseEditModal} className="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 focus:outline-none focus:bg-gray-300 mr-2">Cancel</button>
                            <button onClick={handleSaveEdit} className="px-4 py-2 bg-lime-600 text-white rounded-md hover:bg-lime-800 focus:outline-none focus:bg-lime-700">Save</button>
                        </div>
                    </div>
                </div>
            )}
